...
//creating subnet
resource "aws_subnet" "ajsn1" {
depends_on = [aws_vpc.ajvpc]
...
resource "aws_internet_gateway" "ajig" {
depends_on = [aws_vpc.ajvpc]
  vpc_id = aws_vpc.ajvpc.id
  tags = {
    Name = "IG_terraform"
...
depends_on  = [aws_internet_gateway.ajig,aws_vpc.ajvpc]
  default_route_table_id = aws_vpc.ajvpc.default_route_table_id
   route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.ajig.id
  }
  tags = {
...
depends_on  = [aws_default_route_table.r,aws_subnet.ajsn1]
  subnet_id      = aws_subnet.ajsn1.id
  route_table_id = aws_vpc.ajvpc.default_route_table_id
}




resource "aws_eip" "ajEIP" {
  vpc      = true
}


resource "aws_nat_gateway" "ajNAT" {
  allocation_id = aws_eip.ajEIP.id
  subnet_id = aws_subnet.ajsn1.id
  tags = {
...
resource "aws_route_table" "ajRT" {
depends_on  = [aws_nat_gateway.ajNAT,aws_vpc.ajvpc]
  vpc_id = aws_vpc.ajvpc.id


  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_nat_gateway.ajNAT.id
  }
  tags = {
...
depends_on  = [aws_route_table.ajRT,aws_subnet.ajsn2]
  subnet_id      = aws_subnet.ajsn2.id
  route_table_id = aws_route_table.ajRT.id
}
...
depends_on = [aws_vpc.ajvpc,aws_security_group.mysecbh]
    name        = "wpteraasec"
    vpc_id      = aws_vpc.ajvpc.id
    ingress{
...
depends_on = [aws_vpc.ajvpc,aws_security_group.mysecwp]
    name        = "msteraasec"
    vpc_id      = aws_vpc.ajvpc.id
    ingress{
...
resource "null_resource" "Uploading_key_on_bh_instance"{
    ...
    provisioner "local-exec"{
          command = "scp -i C:/Users/aadit/Downloads/Aaditya.pem C:/Users/aadit/Downloads/Aaditya.pem  ec2-user@${aws_instance.bhmyins.public_ip}:~/"      
      }
  }
